...
describe('squid-policy-provider helpers', () => {
  ...
  describe('socatForwardsDiffer', () => {
    const header = [
      '# Per-agent CDP TCP forwarders — generated by squid-policy-provider.',
      '# Format: <listen-port> <source-ip-or-cidr> <upstream-host> <upstream-port>',
      '',
    ].join('\n');
    const fwd = (port: number, ip: string, hostPort = 9222) =>
      `# some agent\n${port} ${ip}/32 host.docker.internal ${hostPort}`;

    it('is false for byte-identical bodies', () => {
      const body = `${header}\n${fwd(9222, '172.30.0.3')}\n`;
      expect(socatForwardsDiffer(body, body)).toBe(false);
    });

    it('is true when a forwarder is added', () => {
      const prev = `${header}\n${fwd(9222, '172.30.0.3')}\n`;
      const next = `${header}\n${fwd(9222, '172.30.0.3')}\n${fwd(9225, '172.30.0.13')}\n`;
      expect(socatForwardsDiffer(prev, next)).toBe(true);
    });

    it('is true when a forwarder is removed', () => {
      const prev = `${header}\n${fwd(9222, '172.30.0.3')}\n${fwd(9225, '172.30.0.13')}\n`;
      const next = `${header}\n${fwd(9222, '172.30.0.3')}\n`;
      expect(socatForwardsDiffer(prev, next)).toBe(true);
    });

    it('is true from empty (first write) to a populated body', () => {
      expect(socatForwardsDiffer('', `${header}\n${fwd(9222, '172.30.0.3')}\n`)).toBe(true);
    });

    it('ignores comment-only differences (e.g. an agent renamed)', () => {
      const prev = `${header}\n# Old Name (old, ag-x)\n9222 172.30.0.3/32 host.docker.internal 9222\n`;
      const next = `${header}\n# New Name (new, ag-x)\n9222 172.30.0.3/32 host.docker.internal 9222\n`;
      expect(socatForwardsDiffer(prev, next)).toBe(false);
    });

    it('ignores forwarder ordering (compares the set, not the sequence)', () => {
      const a = '9222 172.30.0.3/32 host.docker.internal 9222';
      const b = '9225 172.30.0.13/32 host.docker.internal 9222';
      expect(socatForwardsDiffer(`${header}\n${a}\n${b}\n`, `${header}\n${b}\n${a}\n`)).toBe(false);
    });

    it('is true when only the upstream host port changes', () => {
      const prev = `${header}\n${fwd(9225, '172.30.0.13', 9222)}\n`;
      const next = `${header}\n${fwd(9225, '172.30.0.13', 19222)}\n`;
      expect(socatForwardsDiffer(prev, next)).toBe(true);
    });
  });
});
